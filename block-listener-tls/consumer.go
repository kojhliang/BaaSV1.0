...
type EventsClient struct {
	...
	peerAddress string
    sn string
	peer_tls_root_cert_file string
...
func NewEventsClient(peerAddress string,sn string,peer_tls_root_cert_file string, regTimeout time.Duration, adapter EventAdapter) (*EventsClient, error) {
	var err error
	...
	return &EventsClient{sync.RWMutex{}, peerAddress, sn, peer_tls_root_cert_file,regTimeout, nil, adapter}, err
}

//newEventsClientConnectionWithAddress Returns a new grpc.ClientConn to the configured local PEER.
func newEventsClientConnectionWithAddress(peerAddress string,sn string ,peer_tls_root_cert_file string) (*grpc.ClientConn, error) {
		//var sn string
		//sn="peer0.org1.example.com";
	    //sn="127.0.0.1";
		var creds credentials.TransportCredentials
			var err error
			//creds, err = credentials.NewClientTLSFromFile(config.GetPath("peer.tls.rootcert.file"), sn)
			//creds, err = credentials.NewClientTLSFromFile("/usr/local/balance-transf-1.0.2/artifacts/channel/crypto-config/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt", sn)
	        creds, err = credentials.NewClientTLSFromFile(peer_tls_root_cert_file, sn)
			if err != nil {
...
func (ec *EventsClient) Start() error {
	conn, err := newEventsClientConnectionWithAddress(ec.peerAddress,ec.sn,ec.peer_tls_root_cert_file)
	if err != nil {
