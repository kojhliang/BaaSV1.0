...
app.post('/singleUpload', upload.single('file'), function (req, res, next) {
    ...
    if(fileType !="go"&&fileType !="gz"&&fileType !="zip"&&fileType !="tar"){
        res.send(new RetMsg("100","不能上传此文件格式",""));
        return;
    }
    if(fileType =="go"){
...
app.get("/blockquery", function(req, res) {
    try {
        var ledgerDataStr = JSON.stringify(ledgerData);
        var statsDataStr = JSON.stringify(statsData);
        res.send(render.blockquery({'ledgerData' : ledgerDataStr , 'statsData':statsDataStr}));//jst文件里能够写it.ledgerData,就是从这里传参数过去的
    } catch(e) {
        console.log(" Error retrieving initial hyperledger info "+e);
...
app.get('/transactions/:uuid', function(req, res) {
    try {
        ...
        peerIntf.transactions(channelName,uuid, function (obj) {
            res.send(obj);
        });
    } catch(e) {
        res.send({});
...
app.get('/changeChannel', function(req, res) {
    console.log("*********进入exp-server的changeChannel方法");
    ...
    //立刻触发重新渲染事件
    console.log("***changeChannel,开始showNewBlockData()****");
    showNewBlockData();   //重新渲染区块查询和交易记录页面
    console.log("***changeChannel,开始showNewStatsData()****");
    setTimeout(showNewStatsData,500);  //重新渲染统计图表页面
...
function getInfo(callBk) {
    try {
        if(channelName ==''){
            callBk(ledgerData);  //到这里才会执行getLedgerInfo函数里的参数函数的内容
            return;
        }
        ...
        console.log("********进入getChainInfo，channelName = "+channelName);
        peerIntf.chain(
            channelName,
            function(obj) {
                ...
                var blockFunc = function(obj) {
                    ...
                    if(currHeight == ledgerData.chain.height.low) {
                        ...
                        peerIntf.block(channelName, (currHeight++),blockFunc); //通过这里递归取出所有新增的区块数据
                    }
                ...
                if((!isNaN(currHeight) && currHeight != ledgerData.chain.height.low)||currentBlockHash!= new_currentBlockHash) {
                    //console.log("**************!!!!!!!!!!!!!!!!!!!!!!!!!!!!currHeight != ledgerData.chain.height.low||currentBlockHash!= new_currentBlockHash, do peerIntf.block");
                    peerIntf.block(channelName, 0,blockFunc);//不管是同一管道里有新增的区块还是切换管道获取不同的区块，都要从0从新获取区块信息
                }
 ...
 */
//initial load
//获取最新区块信息，并把最新数据到渲染区块查询页面
var showNewBlockData=function(){
    console.log("***********88888进入showNewBlockData()");
    ...
    getLedgerInfo( function() {
       // console.log("***********ledgerData.chain.height:"+ledgerData.chain.height.low);
        ...
        if((prevHeight != ledgerData.chain.height.low)||currentBlockHash!=new_currentBlockHash) {
            //console.log("***********66666准备更新区块信息，新的区块数据ledgerData.blocks为：");
            //console.log(ledgerData.blocks);
            newData.chain = ledgerData.chain;
            newBlocks = new Array();
            //这里处理逻辑有问题，要考虑替换的情况，以新的20个区块信息替换掉原有的20个区块信息,而不是只是累加
            for(var i = 0; i < ledgerData.chain.height.low; i++)  //把prevHeight改为0
            {
        ...
        if(newData.peers || newData.blocks) {
            io.emit('update', JSON.stringify(newData)); //将会跳转到scripts.js下的160行 socket.on('update', function (msg) {
            console.log("**************准备广播update消息到scripts.js");
...
//获取最新区块信息，并把最新数据到统计图表页面
var showNewStatsData=function(){

    console.log("***********999999进入showNewStatsData(),newBlockArrived:"+newBlockArrived);
    // if(!newBlockArrived &&(flag==null)){
    //     console.log("********进入if(!showNewStatsData)");
    //     return;
    // }
    while(newBlockArrived) {
        newBlockArrived = false;
        console.log("******** 11111设置newBlockArrived为false");
        var endSecs = -1;
        var txnRate = 0;
        var blkRate = 0;
        var txnCount = 0;
        ...
        //console.log(' io ',io);
        io.emit('stats', JSON.stringify(statsData));  //将会跳转到scripts.js下的147行 socket.on('stats', function (msg)
        console.log("**************准备广播stats消息到scripts.js");
        break;
        console.log("**************showNewStatsData end,break****");
    }
}

//开启监听端口，每隔一段事件重新渲染区块查询页面和统计图标页面
var server =null;
var io =null;
var showNewData=function (){
        try {
            ...
            //console.log('***********11111.');
            showNewBlockData();//先初始化，方便页面渲染数据使用
            showNewStatsData();//先初始化，方便页面渲染数据使用
            setInterval(showNewBlockData,10000);
            setInterval(showNewStatsData, 10000);
...
//不断间隔一段时间就更新数据并渲染页面的入口
getLedgerInfo(showNewData);
